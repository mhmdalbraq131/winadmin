    ...
    def _update_style(self):
        if self._selected:
            self.setStyleSheet("""QPushButton {
                background-color: #2a2a4a; color: #ffffff; border: none;
                border-right: 3px solid #4CAF50; text-align: right;
    ...
    def _ensure_page(self, index: int):
        ...
        try:
            ...
            QMessageBox.critical(
                self,
                "خطأ في فتح الصفحة",
                f"تعذر فتح صفحة «{self.PAGE_TITLES[index]}».\n\n{exc}"
            )
            return None
