...
class OptimizedDatabaseManager(DatabaseManager):
    ...
    def _ensure_indexes(self):
        ...
        try:
            self.conn.execute("CREATE INDEX IF NOT EXISTS idx_performance_log_timestamp ON performance_log(timestamp)")
            self.conn.execute("CREATE INDEX IF NOT EXISTS idx_error_log_timestamp ON error_log(timestamp)")
            self.conn.execute("CREATE INDEX IF NOT EXISTS idx_alerts_timestamp_ack ON performance_alerts(timestamp, acknowledged)")
            self.conn.commit()
    ...
    def log_performance(self, cpu, ram_pct, ram_used, ram_total, disk_pct,
        ...
        now = datetime.now()
        if self._last_perf_log_at is not None and (now - self._last_perf_log_at).total_seconds() < self.PERFORMANCE_LOG_INTERVAL_SECONDS:
            return
        self._last_perf_log_at = now
        super().log_performance(cpu, ram_pct, ram_used, ram_total, disk_pct,
                                disk_used, disk_total, net_sent, net_recv)
        ...
        try:
            ...
            cursor = self.conn.execute(
                ...
                SELECT * FROM (
                    ...
                    FROM performance_log p WHERE timestamp >= ?
                )
                WHERE ((rn - 1) % ?) = 0 OR rn = ?
    ...
    def get_performance_history_raw(self, hours=24):
        """السجل الكامل للتصدير؛ لا يستخدم أخذ العينات الخاصة بالواجهة."""
        return DatabaseManager.get_performance_history(self, hours)
